...
function parseArgs(argv = []) {
  const result = {
    ...
    dryRun: false,
    sleepMs: 800,
    providers: ["courtlistener", PRIORITY_FEED_SOURCE, "pacermonitor", "docketalarm", "unicourt"],
    onlyTargetCases: true,
    recentWindowDays: 550
  };
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--limit-per-year" && next) {
      result.limitPerYear = Math.max(Number.parseInt(next, 10) || 80, 1);
      index += 1;
      continue;
    }

    if (arg === "--sample-size" && next) {
      result.sampleSize = Math.max(Number.parseInt(next, 10) || 4000, 100);
      index += 1;
      continue;
    }
...
const scopedCandidates = candidatePool
  .map((item) => ({
    ...item,
  ...
  .filter((item) => {
    ...
    if (!args.onlyTargetCases) {
      return !isPlaceholderCase({ ...item.caseRow, docket_number: item.docket_number });
    }
    return looksLikeTargetCase(item, item.caseRow);
  })
  .map((item) => ({
    ...item,
    date_filed: item.date_filed || item.caseRow?.date_filed || null,
    primary_source: item.primary_source || item.caseRow?.primary_source || null
  }));

const selected = [];
for (const year of args.years) {
  const yearly = scopedCandidates
    .filter((item) => String(item.date_filed || "").startsWith(`${year}-`))
    .map((item) => ({
      ...item,
      selected_providers: buildSelectedProviders(item, allowProviders)
    }))
    .filter((item) => item.selected_providers.length > 0)
    .slice(0, args.limitPerYear);
  selected.push(...yearly);
}

console.log(JSON.stringify({
  years: args.years,
  limitPerYear: args.limitPerYear,
